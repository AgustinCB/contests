...
def solve():
    current_number = SnailfishNumber.parse(eval(sys.stdin.readline().strip()), 1)
    for line in sys.stdin:
        operand = SnailfishNumber.parse(eval(line.strip()), 1)
        current_number = current_number.sum(operand)
